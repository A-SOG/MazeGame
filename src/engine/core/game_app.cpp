...
namespace engine::core {
    ...
    void GameApp::render() {
        ...
        renderer_->clearScreen();// 1. 清除屏幕
        testRenderer();  // 2. 具体渲染代码

        renderer_->present();// 3. 更新屏幕显示
    }
    ...
    {
        resource_manager_->getTexture("assets/Character/Status.png");
        resource_manager_->getFont("assets/fonts/Marker Felt.ttf",16);
        resource_manager_->getSound("assets/audio/bkMusic.mp3");
        resource_manager_->unloadTexture("assets/Character/Status.png");
        resource_manager_->unloadFont("assets/fonts/Marker Felt.ttf", 16);
        resource_manager_->unloadSound("assets/audio/bkMusic.mp3");
    }
    ...
    void GameApp::testCamera()
    {
        ...
        if(key_state[SDL_SCANCODE_W])camera_->move(glm::vec2(0, -1));
        if (key_state[SDL_SCANCODE_S]) camera_->move(glm::vec2(0, 1));
        if (key_state[SDL_SCANCODE_A]) camera_->move(glm::vec2(-1, 0));
        if (key_state[SDL_SCANCODE_D]) camera_->move(glm::vec2(1, 0));
    }
